...
export function Topbar({ onNewClick }: TopbarProps) {
  ...
  const handleTenantSelect = (selectedTenant: Tenant) => {
    ...
    router.push(`/dashboard?tenant=${selectedTenant.slug || selectedTenant.id}`)
    setTenantOpen(false)
  }

  const displayTenant = tenant || { name: "Loading...", brandColor: "#6366f1" }
        ...
        <DropdownMenu open={tenantOpen} onOpenChange={setTenantOpen}>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" className="gap-2 rounded-xl bg-transparent">
              <div className="h-3 w-3 rounded-full" style={{ backgroundColor: displayTenant.brandColor || "#6366f1" }} />
          ...
          <DropdownMenuContent align="start" className="w-56 rounded-xl">
            {isLoading ? (
              ...
              tenants.map((t: Tenant) => (
                ...
                >
                  <div className="h-3 w-3 rounded-full" style={{ backgroundColor: t.brandColor || "#6366f1" }} />
                  <span>{t.name}</span>
                </DropdownMenuItem>
              ))
            ) : (
